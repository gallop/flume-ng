...
public class ContentBuilderUtil {
  ...
  public static void appendField(XContentBuilder builder, String field,
    ...
    System.err.println("-----------mediaType:"+contentType.mediaType());
    if (contentType == null) {
      addSimpleField(builder, field, data);
    } else {
      addComplexField(builder, field, contentType, data);
    }
    //addComplexField(builder, field, XContentType.JSON, data);
  }
  ...
  public static void addComplexField(XContentBuilder builder, String fieldName,
    ...
    System.err.println("---------------into addComplexField---------------------");
    try {
      ...
      parser = XContentFactory.xContent(contentType).createParser(new NamedXContentRegistry(searchModule.getNamedXContents()),DeprecationHandler.THROW_UNSUPPORTED_OPERATION,data);
      while (parser.nextToken() != null) {};

      // If the JSON is valid then include it
      parser = XContentFactory.xContent(contentType).createParser(new NamedXContentRegistry(searchModule.getNamedXContents()),DeprecationHandler.THROW_UNSUPPORTED_OPERATION,data);
      //XContentFactory.xContent(XContentType.JSON).createParser(new String(data, charset)).
      ...
      // as is
      System.err.println("addComplexField 抛出异常："+ex.getMessage());
